...
drive any OpenAI-compatible backend.
"""
import json
import re
...
FOCUS_TEMPLATE = """The user specifically wants clips about the following topic: "{focus}".

Prioritize highlights that strongly relate to this topic; downweight or skip moments that don't. 
When a moment loosely fits, still include it if it has strong viral potential, but prefer on-topic moments for the top scores."""
...
def call_highlight_api(
    ...
    # doesn't have to generate a huge JSON payload.
    target = max(num_clips * 2, 5)
    natural_max = max(2 if is_chunk else 3, int(duration / 90))
    min_clips = min(target, natural_max, 8)
    focus_instruction = (
        FOCUS_TEMPLATE.format(focus=focus.strip()) if focus and focus.strip() else ""
    )
...
def get_highlights(
    ...
    `llm_fn` selects the LLM backend (gateway / openai / gemini).
    `focus` is an optional free-text description of the kind of clips the user
    wants; it's injected into the prompt to bias highlight ranking.
    """
    if llm_fn is None:
